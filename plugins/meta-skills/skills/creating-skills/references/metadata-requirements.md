...
- **Must match directory:** Name must match the parent directory name exactly
...
- [ ] Only valid fields in YAML: `name`, `description`, and optional `license`/`compatibility`/`metadata`/`allowed-tools`
- [ ] No legacy sub-agent fields like `model` or `tools`
